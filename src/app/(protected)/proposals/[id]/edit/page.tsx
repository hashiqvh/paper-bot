...
export default function EditProposal({ params }: EditProposalProps) {
  ...
  const { data: proposalData, isLoading: isLoadingProposal } = useProposal(id);
  ...
  useEffect(() => {
    if (proposalData?.proposal && !isDataLoaded) {
      const proposal = proposalData.proposal;
      
      // Parse content from notes if it exists
      let content = defaultFormData.content;
      try {
        if (proposal.notes) {
  ...
  if (!proposalData?.proposal) {
    return (
      <div className="space-y-6">
        <div className="text-center py-12">
          <p className="text-red-500 mb-4">Proposal not found</p>
      ...
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          onClick={() => {
            ...
            router.push(`/proposals/${id}/preview`);
          }}
            ...
            <Card>
              <CardContent className="pt-6 space-y-3">
                <Button
                  ...
                  disabled={updateProposalMutation.isPending || !isFormValid || isLoadingProposal}
                >
